...
import { Camera, Upload, Heart, AlertCircle, ChevronRight, TrendingUp, Award, Zap, Eye, Brain } from 'lucide-react';
...
export const TongueCoatingDetection: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'detection' | 'results' | 'ai-analysis'>('detection');
  ...
  const analyzeImage = async () => {
    ...
    try {
      if (useAIAgent) {
        // 使用 AI 智能体进行实时分析
        const aiAnalysis = await runAgentAnalysis('tongue-coating', selectedImage);
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-8">
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-2xl font-bold">AI智能分析结果</h2>
            </div>
            <p className="text-gray-700 mb-6">
              基于先进的AI模型，为您提供更精确的舌苔分析结果。
            </p>

            {/* 分析置信度 */}
            <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg border-2 border-purple-200 p-4 flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <Zap className="w-6 h-6 text-purple-600" />
                <div>
                  <p className="font-bold text-gray-900">分析置信度</p>
              ...
              <div className="text-right">
                <p className="text-3xl font-bold text-purple-600">{aiResult.confidence}%</p>
              </div>
            </div>

            {/* 健康评分卡 */}
            <div className="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg border-2 border-amber-200 p-6 mb-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-2xl font-bold text-gray-900">健康评分</h3>
                <div className="text-center">
                  <div className="text-5xl font-bold text-amber-600">{aiResult.healthScore}</div>
                  <p className="text-sm text-gray-600">/ 100</p>
                </div>
              </div>
              <div className="w-full bg-gray-300 rounded-full h-3 overflow-hidden">
                <div
                  className="bg-gradient-to-r from-amber-600 to-orange-600 h-full transition-all duration-1000"
                  style={{ width: `${aiResult.healthScore}%` }}
                />
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="p-4 bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg">
                  <h5 className="font-bold text-gray-900 mb-2">舌质颜色</h5>
                  <p className="text-2xl font-bold text-amber-600">{aiResult.tongueColor}</p>
                </div>
                <div className="p-4 bg-gradient-to-br from-yellow-50 to-amber-50 rounded-lg">
                  <h5 className="font-bold text-gray-900 mb-2">苔质类型</h5>
                  <p className="text-2xl font-bold text-orange-600">{aiResult.coatingType}</p>
                </div>
              </div>
              ...
              <div className="mb-6 p-4 bg-blue-50 rounded-lg border-2 border-blue-200">
                <h5 className="font-bold text-gray-900 mb-2">中医诊断</h5>
                <p className="text-lg text-blue-900 font-semibold">{aiResult.diagnosis}</p>
              </div>
              ...
              <div className="mb-6">
                <h5 className="font-bold text-gray-900 mb-3 text-red-600">主要问题</h5>
                <div className="grid md:grid-cols-2 gap-3">
                  {aiResult.problems.map((problem: string, idx: number) => (
                    <div key={idx} className="flex items-center gap-2 p-3 bg-red-50 rounded-lg">
                      <span className="w-2 h-2 bg-red-500 rounded-full flex-shrink-0"></span>
                      <span className="text-sm text-gray-700">{problem}</span>
                    </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">中医调理方案</h3>
              <div className="space-y-3">
                {aiResult.adjustmentPlan.map((plan: string, idx: number) => (
                  <div key={idx} className="flex items-center gap-3 p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg border border-amber-200">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-r from-amber-600 to-orange-600 flex items-center justify-center text-white font-bold">
                      {idx + 1}
                    </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {aiResult.remedies.map((remedy: any, idx: number) => (
                  <div key={idx} className="border-2 border-amber-200 rounded-lg p-4 hover:shadow-lg transition-all bg-gradient-to-br from-white to-amber-50">
                    <div className="mb-3">
                      <h5 className="font-bold text-gray-900 mb-1">{remedy.name}</h5>
                      <p className="text-xs text-gray-600">{remedy.category}</p>
                    </div>
              ...
              {showFeatures && (
                <div className="mt-4 p-4 bg-indigo-50 rounded-lg border border-indigo-200">
                  <div className="grid md:grid-cols-2 gap-4 mb-4">
                    <div className="p-3 bg-white rounded border border-gray-200">
                      <p className="text-xs text-gray-600">亮度</p>
                      <p className="text-xl font-bold text-gray-900">{aiResult.visualFeatures.brightness}</p>
                      <p className="text-xs text-gray-600 mt-1">(0-255, 越高越亮)</p>
                    </div>
                    <div className="p-3 bg-white rounded border border-gray-200">
                      <p className="text-xs text-gray-600">饱和度</p>
                      <p className="text-xl font-bold text-gray-900">{aiResult.visualFeatures.saturation}%</p>
                      <p className="text-xs text-gray-600 mt-1">(颜色深浅)</p>
                    </div>
                    <div className="p-3 bg-white rounded border border-gray-200">
                      <p className="text-xs text-gray-600">舌苔覆盖</p>
                      <p className="text-xl font-bold text-gray-900">{aiResult.visualFeatures.coatingCoverage}%</p>
                      <p className="text-xs text-gray-600 mt-1">(苔层厚度)</p>
                    </div>
                    <div className="p-3 bg-white rounded border border-gray-200">
                      <p className="text-xs text-gray-600">纹理复杂度</p>
                      <p className="text-xl font-bold text-gray-900">{aiResult.visualFeatures.textureComplexity}</p>
                      <p className="text-xs text-gray-600 mt-1">(表面凹凸)</p>
                    </div>
                  </div>
                  <div className="p-3 bg-white rounded border border-gray-200">
                    <p className="text-xs text-gray-600 mb-2">色调范围</p>
                    <p className="text-lg font-bold text-gray-900">
                      {aiResult.visualFeatures.hueRange.min}° - {aiResult.visualFeatures.hueRange.max}°
                    </p>
                    <p className="text-xs text-gray-600 mt-1">
                      (红色0°/360° 黄色60° 绿色120° 青色180° 蓝色240° 紫色300°)
                    </p>
